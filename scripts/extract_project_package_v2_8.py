#!/usr/bin/env python3
"""Extract and inventory Uroflow Project Package v2.8 ZIP contents."""
...
ZIP_ROOT_PREFIX = "Submission_Build_v2.8/"
...
SELECTED_XLSX = {
    ...
    "Uroflow_Master_Submission_Index_v2.8.xlsx",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to Uroflow_Project_Package_v2.8.zip",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("docs/project-package-v2.8"),
        help="Output directory for inventories and extracted sheets.",
    )
    parser.add_argument(
        "--automation-out",
        type=Path,
        default=Path("scripts/pilot_automation_v2_8"),
...
def main() -> None:
    ...
    with zipfile.ZipFile(args.zip_path, "r") as archive:
        ...
        readme_path = f"{ZIP_ROOT_PREFIX}00_README_and_Indexes/00_README_Submission_Build_v2.8.txt"
        if readme_path in archive.namelist():
            readme_text = archive.read(readme_path).decode("utf-8", errors="replace")
            (output_dir / "source_readme_v2.8.txt").write_text(readme_text, encoding="utf-8")
    ...
    write_csv(
        output_dir / "package_manifest_v2.8.csv",
        [
            "file_name",
            "module",
            "category",
    ...
    write_csv(
        output_dir / "module_summary_v2.8.csv",
        ["module", "category", "file_count", "size_bytes"],
        module_rows,
    )
    write_csv(
        output_dir / "docx_inventory_v2.8.csv",
        ["file_name", "paragraph_count", "table_count", "heading_count", "heading_preview"],
        docx_rows,
    )
    write_csv(
        output_dir / "xlsx_sheet_inventory_v2.8.csv",
