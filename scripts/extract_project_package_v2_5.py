#!/usr/bin/env python3
"""Extract and inventory Uroflow Project Package v2.5 ZIP contents."""
...
ZIP_ROOT_PREFIX = "Submission_Build_v2.5/"
...
SELECTED_XLSX = {
    ...
    "Uroflow_Master_Submission_Index_v2.5.xlsx",
    "Uroflow_DHF_Index_Status_Register_v1.2.xlsx",
    ...
    "Uroflow_EU_MDR_GSPR_Checklist_AnnexI_v1.1_EXECUTED.xlsx",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to Uroflow_Project_Package_v2.5.zip",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("docs/project-package-v2.5"),
        help="Output directory for inventories and extracted sheets.",
    )
    parser.add_argument(
        "--automation-out",
        type=Path,
        default=Path("scripts/pilot_automation_v2_5"),
...
def should_extract_automation(member_name: str) -> bool:
    ...
    if not relative:
        return False
    if relative.startswith("sample/") or relative.startswith("outputs/"):
        return False
...
def main() -> None:
    ...
    with zipfile.ZipFile(args.zip_path, "r") as archive:
        ...
        for info in sorted(members, key=lambda item: item.filename.lower()):
            ...
            is_selected_xlsx = Path(name).name in SELECTED_XLSX and category == "xlsx"
        ...
        readme_path = f"{ZIP_ROOT_PREFIX}00_README_and_Indexes/00_README_Submission_Build_v2.5.txt"
        if readme_path in archive.namelist():
            readme_text = archive.read(readme_path).decode("utf-8", errors="replace")
            (output_dir / "source_readme_v2.5.txt").write_text(readme_text, encoding="utf-8")
    ...
    write_csv(
        output_dir / "package_manifest_v2.5.csv",
        [
            "file_name",
            "module",
            "category",
    ...
    write_csv(
        output_dir / "module_summary_v2.5.csv",
        ["module", "category", "file_count", "size_bytes"],
        module_rows,
    )
    write_csv(
        output_dir / "docx_inventory_v2.5.csv",
        ["file_name", "paragraph_count", "table_count", "heading_count", "heading_preview"],
        docx_rows,
    )
    write_csv(
        output_dir / "xlsx_sheet_inventory_v2.5.csv",
